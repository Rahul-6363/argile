            ...
            <BlurPanel className="bg-white/95 backdrop-blur-md">
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 p-8">
                    ...
                    {product.quickLookImages.map((image: string, index: number) => (
                      <button
                        ...
                        className={`relative w-16 h-16 rounded-lg overflow-hidden border-2 transition-all duration-200 ${
                          currentImageIndex === index ? "border-clay-600" : "border-neutral-200"
                        }`}
                ...
                <div className="flex flex-col">
                        ...
                        {product.swatches.map((swatch: any, index: number) => (
                          <button
                            ...
                            className={`w-8 h-8 rounded-full border-2 transition-all duration-200 relative group ${
                              selectedSwatch === index ? "border-clay-600 scale-110" : "border-neutral-300"
                            }`}
                            style={{ backgroundColor: swatch.color }}
                            onClick={() => setSelectedSwatch(index)}
                          >
                            <div className="absolute bottom-10 left-1/2 transform -translate-x-1/2 bg-clay-800 text-white text-xs px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity duration-200 whitespace-nowrap pointer-events-none">
                              {swatch.name}
                            </div>
                          </button>
                        ))}
                  ...
                  {/* Add to Cart */}
                  <motion.button
                    className="w-full bg-clay-600 text-white py-4 rounded-full font-medium text-lg hover:bg-clay-700 transition-colors duration-200 flex items-center justify-center gap-2"
